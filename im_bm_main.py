...
def parse_args():
    ...
    parser.add_argument(
        "-bm",
        action="store_true",
        help="Enable background model directive generation",
    )

    args = parser.parse_args()

    _require_env(["IM_MODEL_NAME", "IM_BASE_URL", "IM_API_KEY"])
    if args.bm:
        _require_env(["BM_MODEL_NAME", "BM_BASE_URL", "BM_API_KEY"])

    return args


def build_generation_config(model_name: str):
    return {
...
def main():
    ...
    from players import ImBmPlayer
