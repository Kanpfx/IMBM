...
> 状态：研究与演进记录，并非当前运行时的完整接口规范。当前实现已采用紧凑
> XML 风格观测、稳定实体 ID、状态变化和动作历史；稳定 Group ID、区域视野年龄
> 及严格字符预算仍属于后续候选方案。
...
早期方案建议增加稳定的 `GroupRegistry` 和 `SpatialRegistry`：

- 战斗单位按阵营、空地属性、类型和邻近地标聚合。
- Group 只暴露稳定 ID、组成、整体生命值和语义位置。
- 相邻观测使用成员集合的 Jaccard 相似度继承 Group ID；合并保留主要 Group，
  分裂由最大子 Group 继承。
- 动作使用 Group 和地标引用，运行时负责转换为 Unit Tag、Point2 和路径网格。

该方案可以继续降低 Token 数，但会改变当前逐实体动作引用方式，因此本轮不实施。
