...
# ── Strategy (from SunTzu plan_agent, unchanged) ──
strategy_prompt = """
...
# ── Role (tweaked from SunTzu plan_agent) ──
role_prompt = """
...
# ── Plan prompt (SunTzu original + background_request section) ──
...
# ── Plan critic prompt (SunTzu original, unchanged) ──
...
class BmAgent(BaseAgent):
    ...
    def _safe_parse(self, response: str) -> list[str]:
        try:
            ...
            if isinstance(payload, list):
                ...
                # Compat: if LLM outputs a dict, extract string values
    ...
    # ── SunTzu PlanVerifier methods (unchanged) ──
    ...
    def refine_plan(self, obs_text: str, plan: list[str], critic: str, rules: list[str], background_request: str = ""):
    ...
    def refine_plan_until_ready(self, obs_text: str, plan: list[str], rules: list[str], background_request: str = ""):
        for _ in range(self.max_refine_times):
            ...
            plan = self.refine_plan(obs_text, plan, critic, rules, background_request)
        return plan
    ...
    def run(
        ...
        # PlanVerifier: LLM self-correction (from SunTzu, unchanged)
