...
from game.actions.policy import ActionReview, PolicyValidator
...
from llm.agents.model_agent import ModelAgent, ModelResult
...
class LLMGameController:
    ...
    def _start_request(self, bot: Any, iteration: int) -> None:
        ...
        self._pending = asyncio.create_task(
            self.model_agent.run(
                observation.text,
                deepcopy(self.tactic),
                deepcopy(surface.entries),
                deepcopy(feedback),
    ...
    async def _publish_decision(
        ...
        header = (
            ...
            f"t={bot.time_formatted} phase={phase} model={result.latency_ms / 1000:.2f}s "
